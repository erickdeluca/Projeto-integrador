...
function draw() {
  ...
  for (let i = 0; i < obstacle.length; i++) {
    ...
    enemy.obstaclePz = enemy.obstacleZ;
    enemy.enemySx = enemySx;
    enemy.enemySy = enemySy;
    enemy.enemyR = enemyR;

    noStroke();
    fill(255,0, 0);
    circle (enemySx, enemySy, enemyR * 2);
...
function mousePressed() {
  for (let i = 0; i < obstacle.length; i++) {
    if (obstacleClicked(obstacle[i], mouseX, mouseY)) {
      console.log(".");
...
function obstacleClicked(enemy, mx, my) {
  let d = dist(mx - width / 2, my - height / 2, enemy.enemySx, enemy.enemySy);
  return d < enemy.enemyR;
}